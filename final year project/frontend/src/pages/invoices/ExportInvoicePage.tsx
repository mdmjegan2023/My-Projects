...
const empty = () => ({ iExpCode:'', entryDate: new Date().toISOString().split('T')[0], invoiceNo:'', invoiceDate: new Date().toISOString().split('T')[0], clientName:'', poNo:'', totalPair:0, otrCurrencyType:'', otrExchangeRate:0, otrAmount:0, inrExchangeRate:0, inrAmount:0, sgstPercent:0, sgstValue:0, cgstPercent:0, cgstValue:0, igstPercent:0, igstValue:0, remarks:'' });

export const ExportInvoicePage = () => {
  ...
  const load = () => {
    Promise.all([invoiceApi.export.getAll(), masterApi.clients.getAll(), masterApi.currencies.getAll(), invoiceApi.export.docNo()])
      .then(([inv, cl, cu, dn]) => { setRows(inv.data.data); setClients(cl.data.data); setCurrencies(cu.data.data); setForm((f:any) => ({ ...f, iExpCode: dn.data.data })); })
  ...
  const handleSave = async () => {
    if (!form.invoiceNo || !form.clientName || !form.totalPair || !form.otrCurrencyType) { toast.error("Please Fill The Detail's!"); return; }
    setSaving(true);
    try {
      await invoiceApi.export.save(form);
      toast.success('Export Invoice Added.');
      const dn = await invoiceApi.export.docNo();
      setForm({ ...empty(), iExpCode: dn.data.data });
      load();
  ...
  return (
    <Box>
      <PageHeader title="Export Sale Invoice" breadcrumbs={[{ label: 'Bill Entries' }, { label: 'Export Sale Invoice' }]} />
      <Grid container spacing={2.5}>
        <Grid item xs={12} lg={5}>
          <Card><CardContent>
            <Grid container spacing={2}>
              <Grid item xs={6}><TextField fullWidth size="small" label="Doc No" value={form.iExpCode} InputProps={{ readOnly: true }} /></Grid>
              ...
              <Grid item xs={6}><TextField select fullWidth size="small" label="Currency Type *" value={form.otrCurrencyType} onChange={e => set('otrCurrencyType', e.target.value)}><MenuItem value="">Select</MenuItem>{currencies.map((c:any) => <MenuItem key={c.id} value={c.currName}>{c.currName}</MenuItem>)}</TextField></Grid>
              <Grid item xs={6}><TextField fullWidth size="small" label="Exchange Rate" type="number" value={form.otrExchangeRate} onChange={e => set('otrExchangeRate', Number(e.target.value))} /></Grid>
              ...
              <Grid item xs={6}><Button fullWidth variant="outlined" onClick={() => { invoiceApi.export.docNo().then(r => setForm({ ...empty(), iExpCode: r.data.data })); }}>Clear</Button></Grid>
          ...
          <Card><CardContent>
            {loading ? <Box sx={{ textAlign:'center',py:4 }}><CircularProgress /></Box> : (
              <DataTable rows={rows} columns={[
                { key: 'iExpCode', label: 'Doc No' }, { key: 'invoiceNo', label: 'Invoice No' },
                { key: 'invoiceDate', label: 'Date' }, { key: 'clientName', label: 'Client' },
                { key: 'totalPair', label: 'Pairs' },
                { key: 'inrAmount', label: 'INR Amount', render: (v:any) => `₹${Number(v).toLocaleString('en-IN')}` },

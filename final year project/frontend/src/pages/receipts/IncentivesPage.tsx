...
const empty = () => ({ docNo:'', entryDate: new Date().toISOString().split('T')[0], receiptDate: new Date().toISOString().split('T')[0], incentivesType:'', invoiceNo:'', description:'', amount:0, transMode:'', bankName:'', accNo:'', accType:'', bankBranch:'', ifscCode:'', swiftCode:'', remarks:'' });

export const IncentivesPage = () => {
  const [rows, setRows]         = useState<any[]>([]);
  const [form, setForm]         = useState<any>(empty());
  const [invNos, setInvNos]     = useState<string[]>([]);
  ...
  const load = () => {
    Promise.all([receiptApi.incentives.getAll(), masterApi.transactions.getByCat('Retail'), masterApi.banks.getAll(), receiptApi.incentives.docNo()])
      .then(([r, tr, bk, dn]) => {
        setRows(r.data.data); setTrans(tr.data.data); setBanks(bk.data.data);
  ...
  const handleTypeChange = async (type: string) => {
    set('incentivesType', type);
    set('invoiceNo', '');
    try {
      const res = type === 'DBK' ? await invoiceApi.export.openDbkNos() : await invoiceApi.export.openMeisNos();
      setInvNos(res.data.data);
    } catch { setInvNos([]); }
  };
  ...
  const handleSave = async () => {
    if (!form.incentivesType || !form.invoiceNo || !form.amount || !form.transMode || !form.accNo) {
      toast.error("Please Fill The Detail's!"); return;
    }
    setSaving(true);
    try {
      await receiptApi.incentives.save(form);
      toast.success('Incentive Added.');
      const dn = await receiptApi.incentives.docNo();
  ...
  return (
    <Box>
      <PageHeader title="Salexport Incentives" subtitle="DBK & MEIS incentive entries" breadcrumbs={[{ label: 'Receipts' }, { label: 'Salexport Incentives' }]} />
        ...
        <Grid item xs={12} lg={5}>
          <Card><CardContent>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}><TextField select fullWidth size="small" label="Incentives Type *" value={form.incentivesType} onChange={e => handleTypeChange(e.target.value)}><MenuItem value="">Select</MenuItem>{['DBK','MEIS'].map(o => <MenuItem key={o} value={o}>{o}</MenuItem>)}</TextField></Grid>
              <Grid item xs={12}><TextField select fullWidth size="small" label="Invoice No *" value={form.invoiceNo} onChange={e => set('invoiceNo', e.target.value)}><MenuItem value="">Select</MenuItem>{invNos.map(n => <MenuItem key={n} value={n}>{n}</MenuItem>)}</TextField></Grid>
              ...
              <Grid item xs={6}><Button fullWidth variant="outlined" onClick={() => receiptApi.incentives.docNo().then(r => setForm({...empty(), docNo: r.data.data}))}>Clear</Button></Grid>
          ...
          <Card><CardContent>
            {loading ? <Box sx={{ textAlign:'center', py:4 }}><CircularProgress /></Box> : (
              <DataTable rows={rows} columns={[
                ...
                {key:'incentivesType',label:'Type'},{key:'invoiceNo',label:'Invoice No'},
